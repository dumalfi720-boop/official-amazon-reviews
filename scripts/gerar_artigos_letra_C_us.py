...
# -*- coding: utf-8 -*-
"""
gerar_artigos_letra_C_us.py - Gera artigos HTML via OpenRouter para keywords aprovadas LETRA C.

Uso:
  python gerar_artigos_letra_C_us.py --n 5        # testa com 5 artigos
  python gerar_artigos_letra_C_us.py              # processa todos aprovados
  python gerar_artigos_letra_C_us.py --dry        # lista sem criar
  python gerar_artigos_letra_C_us.py --skip-ai    # gera com conteudo placeholder (sem IA)
  python gerar_artigos_letra_C_us.py --force      # sobrescreve artigos existentes

Requer: OPENROUTER_API_KEY em .env (ou env var)
Depois de criar, faz commit+push automatico para Vercel.
"""
...
KW_JSON   = Path(__file__).parent / "C_keywords_para_artigo.json"
...
def gerar_com_ia(client, keyword: str, contexto: str) -> dict:
    prompt = montar_prompt_produto(keyword, contexto)
    modelos = FREE_MODELS
    last_err = None
...
# -- HTML Template (reusa template de A e B) ---------------------------------
...
def make_html(slug, kw, top_kw, nome, link_afiliado, volume, kd, ai_dados=None):
    ...
    ai_dados: dict com 'buying_guide_html' e 'faq' gerados pelo Claude.
              Se None, usa conteudo placeholder estatico.
    """
    ...
    # FAQ: usa Claude se disponivel, senao placeholder
    ...
    # Buying guide: usa Claude se disponivel, senao placeholder
    if ai_dados and ai_dados.get("buying_guide_html"):
      ...
      <ul>
        <li><strong>Quality & Durability:</strong> Look for products built to last with quality materials.</li>
        <li><strong>Customer Reviews:</strong> Check Amazon ratings and comments from verified buyers.</li>
        <li><strong>Price Comparison:</strong> Compare prices across similar products to find the best value.</li>
        <li><strong>Warranty & Support:</strong> Consider warranty coverage and seller support.</li>
        <li><strong>Shipping & Availability:</strong> Check Prime eligibility and estimated delivery times.</li>
      </ul>
      <h3>Our Recommendation for 2026</h3>
      <p>After thorough research, <strong>{nome}</strong> stands out as the top choice for anyone interested in <em>{kw_lower}</em>. It delivers excellent value, quality, and has earned consistently positive feedback from thousands of buyers.</p>
      <h3>Who Should Buy This?</h3>
      <p>This product is ideal for anyone who wants quality <em>{kw_lower}</em>, whether you're a first-time buyer or looking to upgrade your existing setup.</p>"""
        ...
        "@graph": [
            ...
            },
            {
                ...
                "itemListElement": [
                    ...
                    {"@type": "ListItem", "position": 2, "name": "Products", "item": SITE_URL + "/articles/"},
...
<div class="article-wrap">
  ...
  <div class="breadcrumb">
    <a href="{SITE_URL}/">Home</a> &rsaquo; <a href="{SITE_URL}/articles/">Reviews</a> &rsaquo; {kw_title}
  </div>
  ...
  <div class="article-section" id="section-1">
    ...
    <p><strong>{kw_title}</strong> is one of the most searched products in its category, with over {volume:,} monthly searches. Whether you're a beginner or an experienced buyer, understanding what makes a great <em>{kw_lower}</em> can save you time and money.</p>
    <p>{nome} is a standout option that consistently receives high praise from buyers worldwide. It combines quality with value, making it suitable for a wide range of needs.</p>
  ...
  <div class="article-section" id="section-3">
    ...
    <ul style="padding-left:20px;line-height:2">
      ...
      <li>✓ <strong>Selection:</strong> Hundreds of options and variations to choose from</li>
      <li>✓ <strong>Verified Reviews:</strong> Thousands of authentic buyer reviews</li>
...
def main():
    ...
    ap.add_argument('--dry',       action='store_true',  help='So lista, nao cria')
    ...
    ap.add_argument('--skip-serp', action='store_true',  help='Nao busca concorrentes Google')
    ap.add_argument('--cat',       type=str, default='', help='Filtrar por categoria (parcial)')
    ap.add_argument('--vol',       type=int, default=500, help='Volume minimo (padrao 500)')
    ap.add_argument('--kd',        type=int, default=50,  help='KD maximo (padrao 50)')
    args = ap.parse_args()
    ...
    if not args.skip_ai and not args.dry:
        ...
        if not api_key:
            print("ERRO: OPENROUTER_API_KEY nao definida no .env. Use --skip-ai para gerar sem IA.")
            sys.exit(1)
        try:
            ...
            print("ERRO: openai package nao instalado. Execute: pip install openai")
            sys.exit(1)

    # Carrega dados de C_keywords_para_artigo.json
    if not KW_JSON.exists():
        print(f"ERRO: {KW_JSON.name} nao encontrado. Execute primeiro: python gerar_keywords_para_artigo_C.py")
        sys.exit(1)

    data = json.loads(KW_JSON.read_text(encoding="utf-8"))
    ...
    for i, (key, v) in enumerate(pendentes, 1):
        top_kw_raw = v.get('produto') or ''
        top_kw = top_kw_raw if len(top_kw_raw.split()) >= 2 else (v.get('keyword') or top_kw_raw or key)
        nome      = top_kw
        link      = f"https://www.amazon.com/s?k={top_kw.replace(' ', '+')}&tag=1326-20"
        vol       = int(v.get('volume', 0) or 0)
        kd        = v.get('kd', 0)
        slug      = key
        ...
        if client:
            try:
                ...
                if not args.skip_serp:
                    ...
                    urls = buscar_concorrentes(top_kw, is_ebook=False)
                    analises = []
                    for url in urls:
                ...
                print(f"  Gerando conteudo via OpenRouter...")
                ai_dados = gerar_com_ia(client, top_kw, contexto)
        ...
        if slug not in slugs_existentes:
            idx.append({
                ...
                "image": img,
                "category": v.get('categoria', 'Products'),
                "date": TODAY
            })
            slugs_existentes.add(slug)

        # Atualiza keywords_amazon.json
        kw_amazon[slug] = {"nome": nome, "top_kw": top_kw, "volume": vol, "kd": kd, "categoria": v.get('categoria', 'produto'), "link_afiliado": link}
        ...
        # Commit parcial a cada 20 artigos para nao perder progresso
        if criados % 20 == 0:
            ...
            try:
                ...
                subprocess.run(['git', '-C', repo, 'commit', '-m', f"SEO: batch +{criados} articles LETRA C (auto) [{TODAY}]"], check=True, capture_output=True)
    ...
    try:
        ...
        msg = f"SEO: add {criados} new articles LETRA C [{TODAY}]"
        subprocess.run(['git', '-C', repo, 'commit', '-m', msg], check=True)
        subprocess.run(['git', '-C', repo, 'push'], check=True)
        print(f"\nOK {criados} artigos LETRA C criados e publicados no Vercel!")
