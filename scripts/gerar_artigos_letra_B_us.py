...
# -*- coding: utf-8 -*-
"""
gerar_artigos_letra_B_us.py — Gera artigos HTML via OpenRouter para keywords aprovadas LETRA B.

Uso:
  python gerar_artigos_letra_B_us.py --n 5        # testa com 5 artigos
  python gerar_artigos_letra_B_us.py              # processa todos aprovados
  python gerar_artigos_letra_B_us.py --dry        # lista sem criar
  python gerar_artigos_letra_B_us.py --skip-ai    # gera com conteúdo placeholder (sem IA)
  python gerar_artigos_letra_B_us.py --force      # sobrescreve artigos existentes
...
KW_JSON   = Path(__file__).parent / "B_keywords_para_artigo.json"
KW_AMAZON = Path(__file__).parent / "B_keywords_amazon.json"
...
# ── HTML Template (reusa template de A) ────────────────────────────────────
...
def main():
    ...
    # Carrega dados de B_resultado_us.json para montar B_keywords_para_artigo.json
    resultado_us_path = Path(r"C:\projetos\semrush-central\alfabeto\B_resultado_us.json")
    if not resultado_us_path.exists():
        print(f"ERRO: {resultado_us_path.name} não encontrado. Execute primeiro: python rodar_letra_B_us.py")
        sys.exit(1)

    resultado_us = json.loads(resultado_us_path.read_text(encoding="utf-8"))

    # Monta keywords aprovadas de B_resultado_us.json
    data = {}
    for registro in resultado_us:
        for aprovado in registro.get("aprovados", []):
            slug = registro["produto"].lower().replace(" ", "-")
            if slug not in data:
                data[slug] = {
                    "keyword": aprovado["keyword"],
                    "top_kw": registro["produto"],
                    "nome": registro["produto"],
                    "link_afiliado": f"https://www.amazon.com/s?k={registro['produto']}?tag=1326-20",
                    "volume": aprovado["volume"],
                    "kd": aprovado["kd"],
                    "fonte": "semrush",
                    "categoria": "Products",
                }
    ...
    for i, (key, v) in enumerate(pendentes, 1):
        ...
        if criados % 20 == 0:
            ...
            try:
                ...
                subprocess.run(['git', '-C', repo, 'commit', '-m', f"SEO: batch +{criados} articles LETRA B (auto) [{TODAY}]"], check=True, capture_output=True)
    ...
    try:
        ...
        msg = f"SEO: add {criados} new articles LETRA B [{TODAY}]"
        subprocess.run(['git', '-C', repo, 'commit', '-m', msg], check=True)
        subprocess.run(['git', '-C', repo, 'push'], check=True)
        print(f"\nOK {criados} artigos LETRA B criados e publicados no Vercel!")
